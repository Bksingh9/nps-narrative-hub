...
class CrawlerService {
  ...
  async crawlWithCheerio(url, selectors = {}) {
    try {
      ...
      // Extract data based on selectors
      if (selectors.npsScore) data.npsScore = $(selectors.npsScore).text().trim();
      if (selectors.storeName) data.storeName = $(selectors.storeName).text().trim();
      if (selectors.storeCode) data.storeCode = $(selectors.storeCode).text().trim();
  ...
  transformToNPSFormat(crawledData) {
    ...
    if (data.tableData && Array.isArray(data.tableData)) {
      data.tableData.forEach(row => {
        const record = {
          'Store No.': data.storeCode || row['Store'] || row['Store Code'] || row['Location'],
          'Store Name': data.storeName || row['Store Name'] || row['Franchise'],
          'State': row['State'] || row['Province'] || '',
          'Region': row['Region'] || row['Zone'] || '',
          'City': row['City'] || row['Location'] || '',
          'NPS Score': this.parseScore(row['NPS'] || row['Score'] || row['Rating'] || data.npsScore),
          'Response Date': row['Date'] || row['Response Date'] || new Date().toISOString(),
    ...
    const results = await Promise.all(
      jobs.map(job => 
        this.queue.add(async () => {
          const method = job.usePuppeteer ? 'crawlWithPuppeteer' : 'crawlWithCheerio';
