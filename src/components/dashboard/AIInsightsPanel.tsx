...
export function AIInsightsPanel() {
  ...
  const filteredMetrics = escalationMetrics.filter(metric => {
    if (
      ...
      metric.store &&
      metric.store !== filters.selectedStore
    )
      return false;
    if (
      filters.selectedState &&
      metric.state &&
      metric.state !== filters.selectedState
    )
      return false;
    if (
      filters.selectedRegion &&
      metric.region &&
      metric.region !== filters.selectedRegion
    )
      return false;
    return true;
  });
  ...
  const handleAnalyze = async (
    ...
    if (!apiKey || npsData.length === 0) {
      toast.error(
        npsData.length === 0
          ? 'No data available. Please upload CSV data first.'
    ...
    try {
      ...
      if (
        ...
        filteredData = filteredData.filter(row => {
          ...
          if (filters.dateRange.from || filters.dateRange.to) {
            const date =
              row._normalized?.responseDate || row['Response Date'] || row.Date;
            if (date) {
          ...
          if (filters.selectedStore) {
            const store =
              row._normalized?.storeCode || row['Store Code'] || row.Store;
      ...
      if (result.success && result.analysis) {
        ...
        if (analysisType === 'escalation') {
          const metrics = await aiService.generateEscalationMetrics(
          ...
          {!isConnected ? (
            <>
              ...
              <Alert>
                ...
                <AlertDescription>
                  For production, store your key securely in Supabase Edge
                  Function Secrets. Frontend storage is for development only.
                </AlertDescription>
              </Alert>
            </>
          ) : (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            <span>Critical Issues & Escalations</span>
        ...
        <CardContent>
          {!isConnected ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>
                Connect your AI API to enable intelligent escalation analysis
              </p>
            </div>
          ) : (
