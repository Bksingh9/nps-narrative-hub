...
export function AIChatbot() {
  ...
  const analyzeDataContext = () => {
    ...
    const scores = npsData
      .map(r => r._normalized?.nps ?? r['NPS Score'] ?? r.NPS)
    ...
    const uniqueStores = new Set(
      npsData.map(
        r => r._normalized?.storeCode || r['Store Code'] || r['Store No.']
      )
    ).size;
    const uniqueStates = new Set(
      npsData.map(r => r._normalized?.state || r.State)
    ).size;
    const uniqueRegions = new Set(
      npsData.map(r => r._normalized?.region || r.Region)
    ).size;
    ...
    npsData.forEach(r => {
      const store =
        r._normalized?.storeCode || r['Store Code'] || r['Store No.'];
    ...
    const storeNPS = Object.entries(storeScores)
      .map(([store, data]: [string, any]) => {
  ...
  const generateSystemPrompt = () => {
    ...
    return `${
      basePrompt ||
      'You are an NPS Intelligence Assistant analyzing customer feedback data.'
    }
...
- Promoters: ${context.promoters} (${Math.round(
        (context.promoters / context.totalResponses) * 100
      )}%)
- Passives: ${context.passives} (${Math.round((context.passives / context.totalResponses) * 100)}%)
- Detractors: ${context.detractors} (${Math.round(
        (context.detractors / context.totalResponses) * 100
      )}%)
...
${context.storeNPS
  ?.map(
    (s, i) => `${i + 1}. ${s.store}: NPS ${s.nps} (${s.responses} responses)`
  )
  .join('\n')}
`
    : context.summary
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!apiKey || apiKey === 'mock') {
        ...
        if (!context.hasData) {
          mockResponse =
            "📊 I notice you haven't uploaded any NPS data yet. Please upload a CSV file first to get insights about your NPS performance. Once you upload data, I can help you analyze trends, identify top performers, and find areas for improvement.";
        } else if (
          lowerQuery.includes('nps') &&
          (lowerQuery.includes('score') || lowerQuery.includes('current'))
        ) {
          mockResponse = `📈 **Current NPS Performance:**\n\nYour NPS score is **${
            context.npsScore
          }**\n\nBased on ${context.totalResponses} responses across ${
            context.uniqueStores
          } stores.\n\n**Breakdown:**\n• Promoters (9-10): ${Math.round(
            (context.promoters / context.totalResponses) * 100
          )}%\n• Passives (7-8): ${Math.round(
            (context.passives / context.totalResponses) * 100
          )}%\n• Detractors (0-6): ${Math.round(
            (context.detractors / context.totalResponses) * 100
          )}%\n\n${
            context.npsScore > 50
              ? '✅ Excellent performance!'
              : context.npsScore > 0
                ? '📊 Good score with room for growth'
                : '⚠️ Significant improvement opportunity'
          }`;
        } else if (
          ...
          mockResponse = `🏆 **Top Performing Locations:**\n\n1. ${
            context.bestStore?.store || 'Store A'
          } - NPS: ${
            context.bestStore?.nps || 'N/A'
          ...
          mockResponse = `Based on your NPS data:\n\n📊 **Quick Summary:**\n• NPS Score: ${
            ...
            context.uniqueStates
          } states\n\n💡 **Key Insights:**\n• Best Performer: ${
            context.bestStore?.store || 'Top Store'
          } (NPS: ${context.bestStore?.nps || 'N/A'})\n• Promoters: ${Math.round(
            (context.promoters / context.totalResponses) * 100
          )}%\n• Improvement Focus: ${Math.round(
            (context.detractors / context.totalResponses) * 100
        ...
        'https://api.openai.com/v1/chat/completions',
        {
          ...
          body: JSON.stringify({
            ...
            messages: [
              ...
              Recent data for reference: ${JSON.stringify(
      ...
      let fallbackResponse =
        "I'm having trouble connecting to the AI service. However, based on your data:\n\n";
      ...
      if (context.hasData) {
        ...
        fallbackResponse +=
          'Please check your API key in Settings if this persists.';
      } else {
        fallbackResponse +=
          'No data is currently loaded. Please upload CSV data first.';
      }
