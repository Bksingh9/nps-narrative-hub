...
export function RealTimeAlerts({
  ...
  const generateAlerts = async (sourceData: any[]) => {
    ...
    Object.entries(storeGroups).forEach(([store, records]) => {
      ...
      if (storeScores.length > 0) {
        ...
        const storeNPS = Math.round(
          ((storePromoters - storeDetractors) / storeScores.length) * 100
        );
    ...
    if (criticalStores > 0) {
      newAlerts.push({
        ...
        description: `${criticalStores} store${
          criticalStores > 1 ? 's have' : ' has'
        } critically low NPS scores (below -20).`,
        timestamp: new Date(),
        actionable: true,
      });
    }
    ...
    if (recentData.length > 10) {
      ...
      recentNPS = Math.round(
        ((recentPromoters - recentDetractors) / recentScores.length) * 100
      );
    ...
    const uniqueStores = [
      ...new Set(data.map((d: any) => d.storeCode || d['Store Code'])),
    ].length;
    ...
    if (avgResponsesPerStore < 5) {
      newAlerts.push({
        ...
        description: `Average of ${avgResponsesPerStore.toFixed(
          1
        )} responses per store. Consider improving survey distribution.`,
        timestamp: new Date(),
        actionable: false,
      });
    }
    ...
    try {
      const aiAlerts = await openaiService.generateRealTimeAlerts({
        ...
        trend: recentData.length > 0 ? recentNPS - npsScore : 0,
  ...
  if (compact) {
    return (
      <Card className="bg-gradient-to-br from-red-50/50 to-orange-50/50 dark:from-red-900/10 dark:to-orange-900/10 border">
