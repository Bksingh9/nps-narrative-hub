...
export function DataProvider({ children }: { children: ReactNode }) {
    ...
    const initializeData = async () => {
      ...
      try {
        ...
        if (backendLoaded) {
          ...
          console.log('DataContext: Backend not available, checking localStorage');
          ...
          try {
            ...
            if (storedRecords) {
              ...
              if (stored && stored.length > 0) {
                console.log('DataContext: Loaded from localStorage:', stored.length, 'records');
                setRawData(stored);
  ...
  const saveToLocalStorage = () => {
    try {
      ...
      // Save metadata
      localStorage.setItem('nps-metadata', JSON.stringify({
    ...
    try {
      ...
      if (savedData) {
        ...
        if (savedMetadata) {
          ...
          setLastUpdated(metadata.lastUpdated ? new Date(metadata.lastUpdated) : new Date());
        }
    ...
    try {
      ...
      if (statusResult.success && statusResult.totalRecords > 0) {
        ...
        if (result.success && result.data && result.data.length > 0) {
          console.log('DataContext: Successfully loaded', result.data.length, 'records from backend');
  ...
  const loadFilterOptions = async () => { /* placeholder to satisfy TS after function additions */
    try {
      ...
      if (result.success) {
        setFilterOptions({
          ...
          stores: (result.stores || []).map((s: any) => typeof s === 'string' ? s : s.code),
  ...
  const applyFiltersInternal = async (newFilters: Filters): Promise<void> => {
    ...
    if (newFilters.dateFrom || newFilters.dateTo) {
      filtered = filtered.filter(record => {
        ...
        if (newFilters.dateFrom && date < new Date(newFilters.dateFrom)) return false;
        if (newFilters.dateTo && date > new Date(newFilters.dateTo)) return false;
        return true;
      });
    }
    ...
    if (newFilters.state && newFilters.state !== 'all') {
      filtered = filtered.filter(record => 
        (record.state || record.State) === newFilters.state
      );
    }
    
    if (newFilters.city && newFilters.city !== 'all') {
      filtered = filtered.filter(record => 
        (record.city || record.City) === newFilters.city
      );
    }
    
    if (newFilters.region && newFilters.region !== 'all') {
      filtered = filtered.filter(record => 
        (record.region || record.Region) === newFilters.region
      );
    }
    ...
    if (newFilters.npsCategory && newFilters.npsCategory !== 'all') {
      filtered = filtered.filter(record => {
        const score = record.npsScore || record['NPS Score'] || record['On a scale of 0 to 10, with 0 being the lowest and 10 being the highest rating - how likely are you to recommend Trends to friends and family'];
