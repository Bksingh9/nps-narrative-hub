// Data helpers and NPS utilities
// Minimal any typing to keep strict-safe across varying CSV schemas

export function safeGetRecords(): any[] {
  try { return JSON.parse(localStorage.getItem('nps-records') || '[]'); } catch { return []; }
}
export function setRecords(rows:any[]) {
  try { localStorage.setItem('nps-records', JSON.stringify(rows)); } catch {}
...
export function extractDate(r:any) {
  return r?._normalized?.responseDate || r["Response Date"] || r["Survey Date"] || r["Submission Time"] || r["Timestamp"] || r["Date"];
}
export function extractScore(r:any) {
  const v = r?._normalized?.nps ?? r["NPS Score"] ?? r["NPS_Score"] ?? r["NPS"] ?? r["Overall Rating"] ?? r["Rating"] ?? r["Score"];
  const n = typeof v === 'number' ? v : parseFloat(String(v).replace(/[, ]/g,''));
  return Number.isFinite(n) ? Math.max(0, Math.min(10, n)) : undefined;
}
export function extractStore(r:any) {
  return r?._normalized?.storeCode || r["Store Code"] || r["Store_ID"] || r["Outlet Code"] || r["Location Code"] || r["Store"] || r["Outlet"];
}
export function extractState(r:any) {
  return r?._normalized?.state || r["State"] || r["STATE"];
}
export function extractRegion(r:any) {
  return r?._normalized?.region || r["Region"] || r["REGION"] || r["Zone"] || r["Cluster"] || r["Area"];
}
...
export function applyFilters(rows:any[], filters:any) {
  ...
  if (filters?.dateRange?.from || filters?.dateRange?.to) {
    r = r.filter(x => {
      const d = extractDate(x); if (!d) return true;
...
export function groupBy<T>(arr:T[], keyFn:(t:T)=>any) {
  const m = new Map<any,T[]>(); for (const it of arr){ const k = keyFn(it) ?? ''; if(!m.has(k)) m.set(k,[]); m.get(k)!.push(it); } return m;
}
export function dailyBuckets(rows:any[]) {
  const m = new Map<string, any[]>();
  for (const r of rows) {
    const d = extractDate(r); if (!d) continue;
...
// anomaly & benchmark-drops & reasons
...
export function detectBenchmarkDrops(rows:any[]) {
  ...
  const inRange = (d:any, from:number, to:number) => { const t=new Date(d).getTime(); return t>=from && t<=to; };
  const dims = [["store", extractStore],["state",extractState],["region",extractRegion]] as const;
  const out:any[] = [];
  for (const [label, keyFn] of dims) {
    for (const [key, items] of groupBy(rows, keyFn)) {
      if (!key) continue;
      const cur = computeNps(items.filter(r=>inRange(extractDate(r), lastFrom, now)));
      const prv = computeNps(items.filter(r=>inRange(extractDate(r), prevFrom, prevTo)));
      const delta = cur.nps - prv.nps;
      if (prv.total>=20 && cur.total>=20 && delta<=-10) out.push({ dimension:label, key, currentNps:cur.nps, previousNps:prv.nps, delta, currentResponses:cur.total });
    }
  }
  return out;
}
export function topReasons(rows:any[]) {
  const texts = rows.map(r=> r["Comments"]||r["Feedback"]||r["Remark"]||r["Observation"]||r?._normalized?.comments).filter(Boolean);
  const bag = new Map<string,number>();
  for (const t of texts){ for (const w of String(t).toLowerCase().match(/[a-z]+/g)||[]){ if(w.length<4) continue; bag.set(w,(bag.get(w)||0)+1); } }
  ...
  const categories = [...g.entries()].map(([k,rs])=>({ category:k, ...computeNps(rs) })).sort((a,b)=>a.nps-b.nps).slice(0,5);
