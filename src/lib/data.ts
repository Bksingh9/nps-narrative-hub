...
export function extractRegion(r: any) {
  ...
  const existingRegion =
    r['Region'] || r['REGION'] || r['Zone'] || r['Cluster'] || r['Area'];
  if (existingRegion) {
...
export function extractCity(r: any) {
  return (
    r?._normalized?.city || r['City'] || r['CITY'] || r['Town'] || r['Location']
  );
}
...
export function applyFilters(rows: any[], filters: any) {
  ...
  if (filters?.dateRange?.from || filters?.dateRange?.to) {
    r = r.filter(x => {
      ...
      if (filters.dateRange?.from && t < filters.dateRange.from.getTime())
        return false;
      if (filters.dateRange?.to && t > filters.dateRange.to.getTime())
        return false;
      return true;
    });
  }
  if (filters?.selectedStore)
    r = r.filter(x => String(extractStore(x)) === filters.selectedStore);
  if (filters?.selectedState)
    r = r.filter(x => String(extractState(x)) === filters.selectedState);
  if (filters?.selectedRegion)
    r = r.filter(x => String(extractRegion(x)) === filters.selectedRegion);
  if (filters?.selectedCity)
    r = r.filter(x => String(extractCity(x)) === filters.selectedCity);
  return r;
}
...
export function computeNps(rows: any[]) {
  ...
  const avg = total
    ? +(scores.reduce((a: number, b: number) => a + b, 0) / total).toFixed(1)
    : 0;
...
export function detectAnomalies(rows: any[]) {
  ...
  for (const [store, items] of groupBy(rows, extractStore)) {
    const days = dailyBuckets(items).map(([day, rs]) => ({
      day,
      ...computeNps(rs),
    }));
    const mean = days.length
      ? days.reduce((a, b) => a + b.nps, 0) / days.length
      : 0;
    const sd = Math.sqrt(
      days.reduce((a, b) => a + Math.pow(b.nps - mean, 2), 0) /
        (days.length || 1)
    );
    for (const d of days) {
      const z = sd ? (d.nps - mean) / sd : 0;
      if (Math.abs(z) >= 2)
        anomalies.push({ store, day: d.day, nps: d.nps, z: +z.toFixed(2) });
    }
  }
  return anomalies;
}
...
export function detectBenchmarkDrops(rows: any[]) {
  ...
  for (const [label, keyFn] of dims) {
    for (const [key, items] of groupBy(rows, keyFn)) {
      if (!key) continue;
      const cur = computeNps(
        items.filter((r: any) => inRange(extractDate(r), lastFrom, now))
      );
      const prv = computeNps(
        items.filter((r: any) => inRange(extractDate(r), prevFrom, prevTo))
      );
...
export function topReasons(rows: any[]) {
  ...
  const catKey = (r: any) =>
    ...
    r['Segment'] ||
    r['Division'] ||
    r?._normalized?.category;
  const g = groupBy(
    rows.filter(r => catKey(r)),
    (r: any) => catKey(r)
  );
